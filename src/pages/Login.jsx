...
import { Mail, Lock, ArrowRight, ShieldCheck, FileText, CheckCircle2 } from 'lucide-react'
      ...
      <div className="flex w-full lg:w-1/2 flex-col justify-center overflow-y-auto px-5 sm:px-10 lg:px-12 xl:px-16 bg-white flex-shrink-0">
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full max-w-sm mx-auto">
          ...
          <motion.div variants={itemVariants} className="mb-5">
            ...
            <p className="mt-1 text-sm text-slate-500">Securely access your tax documents and messages.</p>
          </motion.div>
          ...
          <motion.form variants={itemVariants} className="space-y-3">
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit"
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white shadow-lg transition-all hover:bg-indigo-600">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <button type="button" className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 hover:border-slate-300">
              ...
              <button type="button" className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 hover:border-slate-300">
                <svg className="w-4 h-4 text-black" fill="currentColor" viewBox="0 0 24 24"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.04 2.26-.86 3.59-.8 1.58.07 2.81.74 3.56 1.83-3.13 1.8-2.65 6.17.46 7.38-.72 1.48-1.57 2.92-2.69 3.76zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.36 2.4-2.02 4.41-3.74 4.25z"/></svg>
                Apple
              </button>
            </div>
          </motion.form>
